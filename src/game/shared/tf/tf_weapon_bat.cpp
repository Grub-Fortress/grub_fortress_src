...
ConVar tf_scout_stunball_old_stun( "tf_scout_stunball_old_stun", "1", FCVAR_REPLICATED, "Old sandman stun." );
...
void CTFStunBall::ApplyBallImpactEffectOnVictim( CBaseEntity *pOther )
{
	...
	if ( flLifeTimeRatio > 0.1f )
	{
		...
		if ( bOldStun )
		{ 
			iStunFlags = ( bMax ) ? TF_STUN_SPECIAL_SOUND | TF_STUN_CONTROLS | TF_STUN_MOVEMENT : TF_STUN_SOUND | TF_STUN_LOSER_STATE | TF_STUN_MOVEMENT;
		}
		else
		{
